...
const TawaMessageRenderer: React.FC<TawaMessageRendererProps> = React.memo(({ 
    ...
    if (isStreaming) {
        ...
        return (
            <div className="w-full flex flex-col gap-2">
                 ...
                 <div className="whitespace-pre-wrap text-base md:text-lg text-stone-700 dark:text-slate-300 leading-relaxed opacity-90 animate-pulse-subtle">
    ...
    const renderContentBlocks = (rawContent: string) => {
        ...
        const highlightEntities = (text: string) => {
            ...
            sortedEntities.forEach((entity) => {
                ...
                // CẢI TIẾN: Không highlight tên người chơi (PC) như một NPC/Thực thể
                ...
                // CẢI TIẾN: Thêm flag 'i' để không phân biệt hoa thường và hỗ trợ tiếng Việt tốt hơn
                // Group 1 ($1) là ký tự trước đó (nếu có), Group 2 ($2) là tên thực thể
    ...
    // Header Row: Thinking Toggle + Edit Button - Always show to allow editing
...
const GameplayScreen: React.FC<NavigationProps> = ({ onNavigate, activeWorld, onUpdateWorld }) => {
  ...
  const [isProxy2Loading, setIsProxy2Loading] = useState(false);
  ...
  useEffect(() => {
    if (isReadyRef.current && dynamicRules.length > 0) {
      syncWorldState();
    }
  }, [dynamicRules, syncWorldState]);
  ...
  const AIMonitor = () => {
    // Determine effective proxies and models
    ...
    let proxy1 = activeProxies.find(p => p.id === settings.activeProxyId);
    if (!proxy1 && (settings?.proxyEnabled || settings?.proxyUrl)) {
        proxy1 = {
            id: 'legacy',
    ...
    let proxy2 = null;
    if (isDualProxy) {
        // Re-assign proxy1 to ensure it's from the activeProxies list if possible
        proxy1 = activeProxies.find(p => p.id === settings.activeProxyId) || activeProxies[0];
        proxy2 = activeProxies.find(p => p.id !== proxy1?.id) || activeProxies[1];
    } else if (settings?.proxyUrl2 && settings?.proxyKey2) {
        // Fallback to legacy proxy 2 if defined
        proxy2 = {
            id: 'legacy2',
            name: settings?.proxyName2 || 'Legacy Proxy 2',
            url: settings?.proxyUrl2,
            key: settings?.proxyKey2,
            model: settings?.proxyModel2 || '',
            isActive: true
        } as any;
    }

    const isProxy1 = !!proxy1?.url && !!proxy1?.key;
    const model1 = (proxy1 && proxy1.model) ? proxy1.model : settings?.aiModel;
    
    const isProxy2 = !!proxy2?.url && !!proxy2?.key;
    const model2 = proxy2?.model;
    ...
    return (
        <div className="p-3 bg-stone-300 dark:bg-slate-900/80 rounded-lg border border-stone-400 dark:border-slate-700 space-y-2 font-mono text-[10px] mt-2">
            <div className="flex justify-between items-center border-b border-stone-400 dark:border-slate-800 pb-1.5">
                ...
                <div className="flex items-center gap-2">
                    {(isLoading || isProxy2Loading) && <div className="w-1.5 h-1.5 rounded-full bg-mystic-accent animate-pulse" />}
                    <span className={(isLoading || isProxy2Loading) ? "text-mystic-accent" : "text-stone-400 dark:text-slate-600"}>
                        {isLoading ? "STORY ENGINE" : isProxy2Loading ? "LOGIC ENGINE" : "IDLE"}
                    </span>
                </div>
            </div>

            <div className="space-y-3">
                {/* PROXY 1 / STORY ENGINE */}
                <div className="space-y-1">
                    <div className="flex justify-between items-center">
                        <span className="text-stone-500 dark:text-slate-500 uppercase font-bold text-[9px]">Engine 1: Story</span>
                        <span className={isProxy1 ? "text-sky-500" : "text-emerald-500"} title={proxy1?.url}>
                            {isProxy1 ? `PROXY (${proxy1?.url || 'Unknown'})` : "DIRECT API"}
                        </span>
                    </div>
                    <div className="flex flex-col pl-2 border-l border-stone-400 dark:border-slate-800">
                        <span className="text-stone-700 dark:text-slate-300 truncate" title={model1}>
                            {model1}
                        </span>
                    </div>
                </div>

                {/* PROXY 2 / LOGIC ENGINE */}
                {(isDualProxy || proxy2) && (
                    <div className="space-y-1 pt-1 border-t border-stone-400/30 dark:border-slate-800/30">
                        <div className="flex justify-between items-center">
                            <span className="text-stone-500 dark:text-slate-500 uppercase font-bold text-[9px]">Engine 2: Logic</span>
                            <span className={isProxy2 ? "text-purple-500" : "text-stone-400 dark:text-slate-600"} title={proxy2?.url}>
                                {isProxy2 ? `PROXY (${proxy2?.url || 'Unknown'})` : "INACTIVE"}
                            </span>
                        </div>
                        {isProxy2 && (
                            <div className="flex flex-col pl-2 border-l border-stone-400 dark:border-slate-800">
                                <span className="text-stone-700 dark:text-slate-300 truncate" title={model2}>
                                    {model2 || "Default Model"}
                                </span>
                            </div>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-stone-400 dark:border-slate-800">
                    ...
                    <div className="flex flex-col items-end">
                        <span className="text-stone-500 dark:text-slate-500 uppercase">Total Tokens</span>
                        <span className="text-mystic-accent font-bold">{formatNumber(totalTokens)}</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col">
                    <div className="flex justify-between items-center cursor-pointer hover:text-mystic-accent transition-colors" onClick={() => setShowTokenDetails(!showTokenDetails)}>
                        ...
                        <span className="text-[8px] opacity-50">{showTokenDetails ? "Ẩn" : "Hiện"}</span>
                    </div>
                ...
                <div className="flex flex-col border-t border-stone-400 dark:border-slate-800 pt-1">
                    <div className="flex justify-between items-center cursor-pointer hover:text-mystic-accent transition-colors" onClick={() => setShowStatsDetails(!showStatsDetails)}>
                        ...
                        <span className="text-[8px] opacity-50">{showStatsDetails ? "Ẩn" : "Hiện"}</span>
                    </div>
  ...
  const handleDeleteSave = async (id: string) => {
    ...
    // Helper function to handle MVU updates from Proxy 2
    const handleMVUUpdates = useCallback((updates: any) => {
        console.log("[GameplayScreen] Nhận MVU updates từ Proxy 2:", updates);
        
        // Cập nhật LSR Table nếu có
        if (updates.parsedEdits) {
            setLsrRuntimeData(prev => {
                const newData = { ...prev };
                let hasChanges = false;
                for (const [tableId, edits] of Object.entries(updates.parsedEdits)) {
                    if (!newData[tableId]) {
                        newData[tableId] = [];
                    }
                    const existingData = newData[tableId];
                    
                    for (const [rowIdStr, rowData] of Object.entries(edits as Record<string, any>)) {
                        const rowId = parseInt(rowIdStr, 10);
                        if (!isNaN(rowId)) {
                            existingData[rowId] = { ...existingData[rowId], ...rowData };
                            hasChanges = true;
                        }
                    }
                }
        ...
        if (updates.setTime) {
            const parts = updates.setTime.split('|').map(Number);
        ...
        } else if (updates.timeCost) {
            const newTime = advanceTime(gameTimeRef.current, updates.timeCost);
            setGameTime(newTime);
            gameTimeRef.current = newTime;
        }

        // Cập nhật MVU ngầm (nếu có)
        if (updates.mvuUpdates && Object.keys(updates.mvuUpdates).length > 0 && onUpdateWorld) {
            onUpdateWorld({
                mvuState: {
                    ...(activeWorldRef.current?.mvuState || {}),
                    ...updates.mvuUpdates
                }
            });
        }
  ...
  const runStreamGeneration = useCallback(async (
      ...
      const effectiveWorldData: WorldData = {
        ...
        if (targetIndex === undefined) {
            ...
            // CẢI TIẾN: Đảm bảo chuyển đến trang chứa tin nhắn đang được tạo lại
            const targetPage = targetIndex <= 10 ? 1 : 1 + Math.ceil((targetIndex - 10) / MESSAGES_PER_PAGE);
        ...
        const stream = gameplayAiService.generateStoryTurnStream(
            ...
            handleMVUUpdates,
            () => setIsProxy2Loading(true),
            () => setIsProxy2Loading(false)
        );
  ...
  }, [syncWorldState, triggerAutosave, handleMVUUpdates]);
  ...
  const handleSendInitial = useCallback(async (currentSettings: AppSettings, world: WorldData, time: GameTime) => {
    ...
    const initialPrompt = startingScenario 
        ? `[SYSTEM COMMAND: START STORY]
Kịch bản khởi đầu: "${startingScenario}"
Hãy viết một mở đầu cực kỳ ấn tượng, sống động và lôi cuốn. Tuân thủ tuyệt đối các quy tắc về thẻ <content>, <incrementalSummary>, <tableEdit>, <set_time> và <branches>.`
        : `[SYSTEM COMMAND: START STORY]
Hãy bắt đầu câu chuyện một cách tự nhiên và lôi cuốn nhất dựa trên bối cảnh thế giới và nhân vật đã thiết lập. Hãy thiết lập bối cảnh hiện tại một cách sống động. Tuân thủ tuyệt đối các quy tắc về thẻ <content>, <incrementalSummary>, <tableEdit>, <set_time> và <branches>.`;
 ...
 }, [runStreamGeneration, processAIResponse, tawaPresetConfig, handleMVUUpdates]);
  ...
  const renderSidebarContent = () => {
      ...
      <div className="h-full flex flex-col bg-stone-300 dark:bg-mystic-900 shadow-xl">
              ...
              <div className="md:hidden p-2 space-y-3 bg-stone-400/20 dark:bg-slate-800/20 rounded-lg border border-stone-400/50 dark:border-slate-700/50 mb-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                      ...
                      <Button 
                          ...
                          disabled={isLoading || isProxy2Loading || !history.some(m => m.role === 'model')} 
          ...
          <div className="p-1 border-t border-stone-400 dark:border-slate-800 bg-stone-200 dark:bg-mystic-900/95 flex flex-row gap-1 mt-auto shrink-0">
              ...
              <Button variant="outline" className="flex-1 text-[12px] h-9 px-1 justify-center border border-stone-400 dark:border-slate-700 font-mono font-bold" icon={<Save size={12}/>} onClick={handleManualSave} isLoading={isSaving} disabled={isLoading || isProxy2Loading} title="Lưu thủ công và tải file (.json)">Lưu</Button>
    ...
    <div className="flex h-full w-full bg-stone-300 dark:bg-mystic-900 font-sans overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full relative z-10 min-w-0">
                    ...
                    {isProxy2Loading && (
                        <div className="flex flex-col items-center justify-center p-6 space-y-3 animate-fade-in w-full border-t border-stone-400 dark:border-slate-800/30">
                            ...
                            <span className="text-sm font-medium text-stone-500 dark:text-slate-400 animate-pulse">
                                Đang cập nhật trạng thái thế giới...
                            </span>
                        </div>
                    )}
                    {isLoading && !isProxy2Loading && (history.length === 0 || !history[history.length - 1]?.text) && (
                        <div className="flex flex-col items-center justify-center p-6 space-y-3 animate-fade-in w-full border-t border-stone-400 dark:border-slate-800/30">
                            <Loader2 className="w-8 h-8 text-mystic-accent animate-spin" />
                            <span className="text-sm font-medium text-stone-500 dark:text-slate-400 animate-pulse">
            ...
            <div className="bg-stone-300 dark:bg-mystic-900 border-t border-stone-400 dark:border-slate-800 z-20 shrink-0 flex flex-col shadow-[0_-5px_15px_rgba(0,0,0,0.05)] dark:shadow-[0_-5px_15px_rgba(0,0,0,0.2)]">
                ...
                <GameInput 
                    onSend={handleSend}
                    isLoading={isLoading || isProxy2Loading}
                ...
                >
                    ...
                    >
                        <RefreshCw size={14} className={(isLoading || isProxy2Loading) ? "animate-spin" : ""} />

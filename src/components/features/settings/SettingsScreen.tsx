                    ...
                    <div className="space-y-8">
                        <section className="space-y-4">
                            ...
                            {settings.proxyEnabled && (
                                <div className="flex justify-between items-center bg-stone-100 dark:bg-slate-800/50 p-4 rounded-lg border border-stone-300 dark:border-slate-700">
                                    <div>
                                        <h4 className="text-sm font-medium text-slate-800 dark:text-slate-200">Dual Proxy MVU (Thử nghiệm)</h4>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">Sử dụng 2 Proxy: Proxy 1 viết truyện, Proxy 2 xử lý logic ngầm. Yêu cầu ít nhất 2 Proxy đang bật (ACTIVE).</p>
                                    </div>
                                    {(() => {
                                        const activeProxiesCount = settings.proxies?.filter(p => p.isActive).length || 0;
                                        const canEnableDualProxy = activeProxiesCount >= 2;
                                        return (
                                            <button
                                                onClick={() => {
                                                    if (canEnableDualProxy) {
                                                        handleChange('useDualProxyMVU', !settings.useDualProxyMVU);
                                                    }
                                                }}
                                                disabled={!canEnableDualProxy}
                                                className={`relative w-10 h-5 rounded-full transition-colors ${!canEnableDualProxy ? 'opacity-50 cursor-not-allowed bg-slate-400' : (settings.useDualProxyMVU ? 'bg-mystic-accent' : 'bg-slate-400 dark:bg-slate-600')}`}
                                                title={!canEnableDualProxy ? "Cần ít nhất 2 Proxy đang hoạt động (ACTIVE)" : (settings.useDualProxyMVU ? "Đang bật" : "Đang tắt")}
                                            >
                                                <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${settings.useDualProxyMVU && canEnableDualProxy ? 'left-6' : 'left-1'}`} />
                                            </button>
                                        );
                                    })()}
                                </div>
                                            ...
                                            <div className="flex justify-between items-center">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex items-center gap-1.5 mr-2">
                                                        <button 
                                                            onClick={() => {
                                                                const newIsActive = !proxy.isActive;
                                                                const updatedProxies = settings.proxies.map(p => 
                                                                    p.id === proxy.id ? { ...p, isActive: newIsActive } : p
                                                                );
                                                                
                                                                let newActiveId = settings.activeProxyId;
                                                                // Nếu tắt proxy đang là Primary, chọn cái khác làm Primary nếu có thể
                                                                if (!newIsActive && settings.activeProxyId === proxy.id) {
                                                                    const nextActive = updatedProxies.find(p => p.isActive);
                                                                    newActiveId = nextActive ? nextActive.id : undefined;
                                                                }

                                                                // Nếu số lượng proxy active < 2, tự động tắt Dual Proxy
                                                                const activeCount = updatedProxies.filter(p => p.isActive).length;
                                                                const useDualProxyMVU = activeCount >= 2 ? settings.useDualProxyMVU : false;
                                                            ...
                                                            className={`w-9 h-5 rounded-full relative transition-colors ${proxy.isActive ? 'bg-emerald-500' : 'bg-slate-400 dark:bg-slate-600'}`}
                                                            title={proxy.isActive ? "Proxy đang hoạt động" : "Proxy đang tắt"}
                                                        >
                                                            <div className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${proxy.isActive ? 'left-5' : 'left-1'}`} />
                                                        </button>
                                                        <span className="text-[9px] font-bold text-slate-500 uppercase">{proxy.isActive ? 'ON' : 'OFF'}</span>
                                                    </div>

                                                    <button 
                                                        onClick={() => {
                                                            // Khi SELECT làm Primary, tự động bật proxy đó lên
                                                            const updatedProxies = settings.proxies.map(p => 
                                                                p.id === proxy.id ? { ...p, isActive: true } : p
                                                            );
                                                            handleMultipleChanges({
                                                                proxies: updatedProxies,
                                                        ...
                                                        disabled={!proxy.url || !proxy.key}
                                                        className={`flex items-center gap-2 px-2 py-1 rounded text-[10px] font-bold border transition-all ${settings.activeProxyId === proxy.id ? 'bg-mystic-accent text-white border-mystic-accent' : 'bg-slate-500/10 text-slate-500 border-slate-500/20 hover:bg-slate-500/20'} ${(!proxy.url || !proxy.key) ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                    >
                                                        {settings.activeProxyId === proxy.id ? <CheckCircle2 className="w-3 h-3" /> : null}
                                                        {settings.activeProxyId === proxy.id ? 'PRIMARY' : 'SELECT AS PRIMARY'}
                                                    </button>

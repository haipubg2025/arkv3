 ...
 */
export const buildMVULogicPrompt = (
  ...
  return `
You are the Game Master (Logic Engine) for a text-based RPG.
Your ONLY job is to analyze the recent story events and update the game's internal state.
...
[YOUR TASK]
Based on the events above, you must output the state updates.
You MUST output your response in the following format:

1. If time has passed, output <time_cost>X</time_cost> where X is minutes.
2. If the time needs to be explicitly set (e.g., first turn), output <set_time>year|month|day|hour|minute</set_time>.
3. If the LSR table data needs updating (e.g., character relationships, locations, inventory), output <tableEdit>...</tableEdit> using the standard LSR format (#ID Name|0:Val|1:Val).
4. If there are other dynamic variables (like affection, mood, health), output a JSON block wrapped in \`\`\`json ... \`\`\`.

Example Output:
<time_cost>15</time_cost>
<tableEdit>
#0 Current Information|0:10:45 AM, 15/04/1204|1:Border Tea Party
#1 Recent Characters|0:Tran Thien Vu|1:5|2:Diplomatic Negotiation
</tableEdit>
\`\`\`json
{
  "updates": {
    "affection_TranThienVu": "+5",
    "player_mood": "Cautious"
  }
}
\`\`\`

CRITICAL: DO NOT output any narrative text, dialogue, or explanations. ONLY output the tags and JSON block.
`;
};
...
export const buildGameplaySystemPrompt = (
  ...
  isStoryOnly: boolean = false // NEW: Cờ cho cơ chế 2 Proxy
) => {
  ...
  // TURN 0 WORD COUNT LIMIT (User Request: 1000 words to prevent overload)
  if (turnCount === 0) {
      minWords = 800;
      maxWords = 1200;
  } else if (activeOutputLength.id === 'custom') {
...
${rulesContent}
  ...
  if (turnCount <= 1) {
      ...
      // TURN ZERO SPECIFIC DIRECTIVE
      const turnZeroDirective = turnCount === 0 ? `
<TURN_ZERO_ULTIMATUM>
⚠️ CRITICAL: This is TURN 0. You are INITIALIZING the entire story world.
1. **NO META-TALK**: Absolutely DO NOT say "Certainly", "Okay", "Here is your story", or any introductory meta-commentary. Start roleplaying IMMEDIATELY.
2. **ESTABLISH THE SCENE**: Describe the environment, the atmosphere, and the MC's current state in vivid detail.
3. **INITIALIZE ALL SYSTEMS**: You MUST output <tableEdit>, <set_time>, and <incrementalSummary> tags.
4. **NO REPETITION**: Do NOT repeat the system's "Start the story" prompt.
</TURN_ZERO_ULTIMATUM>` : "";
...
⚠️ IMPORTANT: This is the BEGINNING of the story.${turnZeroDirective}
...
${!isStoryOnly ? `   - The <table_stored> or <tableEdit> tag MUST be AFTER the </content> tag and BEFORE the <branches> tag.
   ...
   - **CRITICAL**: NEVER put story narrative, descriptions, dialogue${!isStoryOnly ? ', or LSR DATA (like #0 Current Information|0:...)' : ''} inside the <branches> tag.
...
${!isStoryOnly ? `   - All world status information MUST be updated via the <tableEdit> tag in LSR format (#ID Name|0:Val|1:Val).
...
${!isStoryOnly ? `3. SPECIAL NOTE: If the current time is "January 01, 2024", this is a TEMPORARY (placeholder) timestamp. 
...
5. BRANCHING RULES (<branches>):
   ...
   - **STRICT SEPARATION**: The <branches> tag is ONLY for action choices. NEVER put story narrative, descriptions, dialogue${!isStoryOnly ? ', or LSR DATA (like #0 Current Information|0:...)' : ''} inside the <branches> tag. All story content MUST be in the <content> tag.
...
${!isStoryOnly ? `     <time_cost>15</time_cost>` : ''}
     <branches>
     [15] Persuade the guard...
     [30] Find a way around the alley...
     </branches>
${!isStoryOnly ? `6. Always end the response by providing all necessary system tags in order: </content> -> <time_cost> -> <set_time> (if needed) -> <branches>.` : ''}
`,
        source: 'GameStatus'
    });
  }
